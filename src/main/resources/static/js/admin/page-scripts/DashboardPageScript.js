...
function initDashboardPage() {
    ...
    form.addEventListener("submit", async function (e) {
        ...
        await fetchDashboardList(formData);
    });

    // 공지 목록을 비동기적으로 가져오는 함수
    async function fetchDashboardList(formData) {
        try {
            const response = await fetch("/admin/dashboards/dashboard_list", {
                method: "POST",
                body: formData,
            });
            if (!response.ok) {
