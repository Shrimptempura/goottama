...
public class ReviewServiceImpl implements ReviewService {
    ...
    public Long insertPolyReview(ReviewDto dto) {
        if (dto == null) {
            log.error("ReviewServiceImpl : 공통 review dto 누락 - dto: {}", dto);
            throw new IllegalArgumentException("리뷰 dto가 없습니다.");
        }
        log.info("ReviewServiceImpl - 상위 리뷰 생성 시작 - dto: {}", dto);

        try {
            int started = reviewDao.insertPolyReview(dto);
            if (started == 0 || dto.getReviewId() == null) {
                log.error("ReviewServiceImpl - 상위 리뷰 생성 실패 - dto: {}", dto);
                throw new IllegalStateException("상위 리뷰 생성 실패");
            }
            log.info("ReviewServiceImpl - 상위 리뷰 생성 성공 - dto: {}", dto);
            return dto.getReviewId();
        } catch (DataAccessException e) {
            log.error("ReviewServiceImpl - DB 오류, 상위 리뷰 저장 실패 - dto: {}", dto, e);
    ...
    public ReviewDto selectDefaultFieldById(Long reviewId) {
        if (reviewId == null) {
            log.warn("ReviewServiceImpl - 상위 리뷰 ID가 없습니다. - reviewId: {}", reviewId);
            throw new IllegalArgumentException("상위 리뷰 ID가 없습니다.");
        }
        log.info("ReviewServiceImpl - 상위 리뷰 default 값만 조회 - reviewId: {}", reviewId);

        try {
            ReviewDto selected = reviewDao.selectDefaultFieldById(reviewId);
            if (selected == null) {
                log.error("ReviewServiceImpl - 상위 리뷰 default 조회 실패(없음) - reviewId: {}", reviewId);
                throw new IllegalStateException("default 값 조회 실패");
            }
            return selected;
        } catch (DataAccessException e) {
            log.error("ReviewServiceImpl - DB 오류, default 값 조회 실패 - reviewId: {}", reviewId, e);
    ...
    public ReviewDto selectAllFieldById(Long reviewId) {
        if (reviewId == null) {
            log.warn("ReviewServiceImpl - 상위 리뷰 아이디가 없습니다. - reviewId: {}", reviewId);
        ...
        try {
            ...
            log.error("ReviewServiceImpl - DB 오류, 상위 리뷰 전체 조회 실패 - reviewId: {}", reviewId, e);
