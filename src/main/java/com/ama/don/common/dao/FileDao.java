...
public interface FileDao {
	...
	void deleteByTargetAndUploader(@Param("file_uploader") String uploader, @Param("targetType") String string,
			@Param("targetId") long target_id);
	...
	// 특정 temp_id는 보존하고 나머지 음수 삭제
	void deleteTempFilesExcept(@Param("target_type") TargetType targetType, @Param("file_uploader") Long userId,
			@Param("keep_temp_id") Long keepTempId);
	...
	// 인테리어사용, 리스트 조회
	List<FileDto> interFindByTarget(@Param("targetType") TargetType targetType, @Param("targetId") Long targetId);

	// 인테리어사용, fileId로 단건 조회
	FileDto interFindById(@Param("fileId") Long fileId);

	// 인테리어사용, 썸네일 1장 찾기
	FileDto interFindThumbnail(@Param("targetType") TargetType targetType, @Param("targetId") Long targetId);

	// 인테리어사용, fileId로 파일 삭제
	int interDeletedById(@Param("fileId") Long fileId);

	// 인테리어 사용, 타겟으로 모두 삭제
	int interDeleteAllByTarget(@Param("targetType") TargetType targetType, @Param("targetId") Long targetId);
